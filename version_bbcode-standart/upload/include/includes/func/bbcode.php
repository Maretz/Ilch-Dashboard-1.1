    ...
    for($i=0;$i<count($imgRs[0]);$i++) {
      ...
      if (isset($endung[$size[2]]) OR !is_array($size)) {
        ...
        if ( $breite > $max_breite ) {
          ...
          $er = '<a class="dbbcodenone" href="'.$imgstr.'" target="_blank"><img height="'.$neueHoehe.'" width="'.$neueBreite.'" style="border: none;" src="'.$imgstr.'" /></a>';
        }
      } 
...
function bbcode_smiles ($s) {
  ...
  foreach ($global_smiles_array as $k => $v) {
    ...
    $s = str_replace($k,'<img class="dbbcodesmilie" src="include/images/smiles/'.$url.'" border="0" alt="'.$emo.'" title="'.$emo.'" />',$s);
  }
  return ($s);
}
