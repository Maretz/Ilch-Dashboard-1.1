    ...
    function _smileys($string) {
        if(!is_null($this->smileys) && is_array($this->smileys)) {
            if($this->permitted['smileys'] == true) {
                ...
                foreach ($this->smileys as $icon => $info) {
                    list($emo, $url) = explode('#@#-_-_-#@#', $info);
                    $string = str_replace($smileystart.$icon.$smileyend, '<img class="dbbcodesmilie" src="include/images/smiles/'.$url.'" border="0" alt="'.$icon.'" title="'.$emo.'" />', $string);
                }
...
//    }
    ...
    $image = '<a class="dbbcodenone" href="'.$string.'" target="_blank"><img src="'.$string.'" alt="" title="" border="0" width="'.$this->info['ScreenMaxBreite'].'" height="'.$this->info['ScreenMaxHoehe'].'" '.$float.'/></a>';
    return $image;
    }
    ...
    //> Info Boxen.
    function _inbox($infoboxe,$inhaltbox) {
        $infoboxe = strtolower($infoboxe);

        if($infoboxe == "success") {
            $infb = "<div class=\"bbcodesuccess\">$inhaltbox</div>";
        }

        if($infoboxe == "info") {
            $infb = "<div class=\"bbcodeinfo\">$inhaltbox</div>";
        }

        if($infoboxe == "warning") {
            $infb = "<div class=\"bbcodewarning\">$inhaltbox</div>";
        }

        if($infoboxe == "danger") {
      $infb = "<div class=\"bbcodedanger\">$inhaltbox</div>";
    }
    ...
    function _flash($url, $options){
        ...
        return '<object class="dbbcodenone" classid="CLSID:D27CDB6E-AE6D-11cf-96B8-444553540000" width="'.$width.'" height="'.$height.'"'.
    ...
    function parse($string) {
        ...
        //> Darf BB Code [SUP] dekodiert werden?
        if($this->permitted['sup'] == true) {
            //> Format: [sup]xxx[/sup]
            $this->pattern[] = "%\[sup\](.+)\[\/sup\]%Uis";
            $this->replace[] = "<sup style=\"margin-left:2px;\">\$1</sup>";
        }

        //> Darf BB Code [SUB] dekodiert werden?
        if($this->permitted['sub'] == true) {
            //> Format: [sub]xxx[/sub]
            $this->pattern[] = "%\[sub\](.+)\[\/sub\]%Uis";
            $this->replace[] = "<sub style=\"margin-left:2px;\">\$1</sub>";
        }
      ...
      //> Format: [shot=left|right]xxx.de[/screenshot]
        ...
        //> Darf BB Code [PROGRESS] dekodiert werden?
        if($this->permitted['progress'] == true) {
            //> Format: [progress=xxx]xxx[/progress]
            $this->pattern[] = "%\[progress=([0-9]|[1-9][0-9]|100)\](.+)\[\/progress\]%Uis";
           $this->replace[] = "<div style=\"background-color:".$this->info['ProgressBackground'].";margin: 5px 0px;border:1px solid ".$this->info['ProgressBorder'].";width: ".$this->info['ProgressWidth'].";\"><span style=\"display:inline-block;height:100%;width:$1%;min-width:30px;padding: 4px 0px;background-color:".$this->info['ProgressBar'].";color:".$this->info['ProgressFontColor'].";text-align:center;\">\$2</span></div>";

        }

         //> Darf BB Code [BLINK] dekodiert werden?
        if($this->permitted['blink'] == true) {
            //> Format: [blink]xxx[/blink]
            $this->pattern[] = "%\[blink\](.+)\[\/blink\]%Uis";
            $this->replace[] = "<span class=\"bbcode_blink\">\$1</span>";
        }

        //> Darf BB Code [TOOL] dekodiert werden?
        if($this->permitted['tool'] == true) {
            //> Format: [tool=xxx]xxx[/tool]
            $this->pattern[] = "%\[tool=(.*)\](.+)\[\/tool\]%Uis";
           $this->replace[] = "<span title=\"$1\" rel=\"bbcodetooltip\">$2</span>";

        }

        //> Darf BB Code [marquee] dekodiert werden?
        if($this->permitted['marquee'] == true) {
            //> Format: [marquee=xxx]xxx[/marquee]
            $this->pattern[] = "%\[marquee=(.*)\](.+)\[\/marquee\]%Uis";
           $this->replace[] = "<marquee behavior=\"ALTERNATE\" WIDTH=\"$1\" SCROLLAMOUNT=\"".$this->info['LauftextSpeed']."\" style=\"background-color:".$this->info['LauftextBG'].";color:".$this->info['LauftextColor'].";\">\$2</marquee>";

        }

         //> Darf BB Code [GOOGLE] dekodiert werden?
        if($this->permitted['google'] == true) {
            //> Format: [google]xxx[/google]
            $this->pattern[] = "%\[google\](.+)\[\/google\]%Uis";
            $this->replace[] = "<a href=\"http://www.google.com/search?q=$1\" target=\"_blank\" title=\"Suche nach $1\" style=\"cursor:help;border-bottom: 1px dotted\">\$1</a>";
        }
        ...
        $twitchtrue = true;
         ...
         //> Darf BB Code [TWITCH] dekodiert werden?
        if($this->permitted['twitch'] == true) {
            //> Format: [twitch]xxx[/twitch]
            $this->pattern[] = "%\[twitch\](.+)\[\/twitch\]%Uis";
            $this->replace[] = "<iframe class=\"dbbcodenone\" src=\"https://player.twitch.tv/?channel=$1\" frameborder=\"0\" allowfullscreen=\"true\" scrolling=\"no\" width=\"".$this->info['TwitchW']."\"   height=\"".$this->info['TwitchH']."\"></iframe>";
        }

         //> Darf BB Code [HITBOX] dekodiert werden?
        if($this->permitted['hitbox'] == true) {
            //> Format: [hitbox]xxx[/hitbox]
            $this->pattern[] = "%\[hitbox\](.+)\[\/hitbox\]%Uis";
            $this->replace[] = "<iframe class=\"dbbcodenone\" width=\"".$this->info['TwitchW']."\" height=\"".$this->info['TwitchH']."\" src=\"http://hitbox.tv/#!/embed/$1?autoplay=".$this->info['PlayerONOFF']."\" frameborder=\"0\" allowfullscreen></iframe>";
        }

        //> Darf BB Code [INBOX] dekodiert werden?
        if($this->permitted['inbox'] == true) {
            //> Format: [infobox=xxx]xxx[/infobox]
            $this->pattern[] = "%\[inbox=(success|info|warning|danger)\](.+)\[\/inbox\]%eUis";
            $this->replace[] = "\$this->_inbox('\$1','\$2')";
        }
        
         //> Darf BB Code [FBPOST] dekodiert werden?
        if($this->permitted['fbpost'] == true) {
            //> Format: [fbpost]xxx[/fbpost]
            $this->pattern[] = "%\[fbpost\](.+)\[\/fbpost\]%Uis";
            $this->replace[] = "<div class=\"fb-post dbbcodenone\" data-href=\"$1\" data-width=\"".$this->info['TweetW']."\"><div class=\"fb-xfbml-parse-ignore\"></div></div>";
        }
        
         //> Darf BB Code [TWEET] dekodiert werden?
        if($this->permitted['tweet'] == true) {
            //> Format: [tweet]xxx[/tweet]
            $this->pattern[] = "%\[tweet\](.+)\[\/tweet\]%Uis";
            $this->replace[] = "<blockquote class=\"twitter-tweet dbbcodenone\" data-lang=\"de\"  data-width=\"".$this->info['TweetW']."\"><a href=\"$1\"></a></blockquote>";
        }        

         //> Darf BB Code [GPOST] dekodiert werden?
        if($this->permitted['gpost'] == true) {
            //> Format: [gpost]xxx[/gost]
            $this->pattern[] = "%\[gpost\](.+)\[\/gpost\]%Uis";
            $this->replace[] = "<div class=\"g-post dbbcodenone\" data-href=\"$1\" data-width=\"".$this->info['TweetW']."\"></div>";
        }  
